---
title: Retrieve a consolidation's definition
...
Changesets are used in this workflow. For information on how to create and use changesets see [Changesets](/docs/common-workflows/modeling/changesets.md). If you plan to use the response of `GET /api/model/consolidation/{consolidationId}` to create a new consolidation or update the consolidation's definition, it is recommended to associate all requests to one changeset.
