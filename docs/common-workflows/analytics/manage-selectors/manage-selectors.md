...
The following selector types are supported (depending on the product version):
...
- Parameter selector (MicroStrategy 2021 Update 12 or later)
...
| [POST /api/dossiers/instances](https://demo.microstrategy.com/MicroStrategyLibrary/api-docs/index.html#/Dossiers%20and%20Documents/createDossierInstance_2)                                            | Set selectors in the current chapter based on a new dossier instance                                                                              |
...
The enhancement for the setting dashboard filter API does not break compatibility since the existing selection setting API is enhanced to support two filter selection json formats: the old format and new format.

The existing setting dashboard filter API is shown below. However, it is suggested that you use the new format of the filter setting body input. This new format is consistent with the filter definition in the output for the GET dashboard definition APIs.
...
The diagrams below outlines common workflows for using APIs for dashboard selector functionalities.
