# Run `python3 scripts/convert_title_to_sentence_case.py` at root level
...
def convert_word_to_proper_case(words, i, word, should_capitalize=True):
...
def convert_title_to_sentence_case(title, should_capitalize=True):
  """
  ...
  while i < len(words):
    ...
    if word.startswith('`'):
      ...
      while i < len(words) and is_checking_markdown_link:
        ...
        if not is_word_in_backtick and not is_word_in_link:
          words[i] = convert_word_to_proper_case(words, i, words[i], should_capitalize)
          ...
          if ']' in words[i]:
            ...
            if word_before_link[-1] != '`' and 'api/' not in word_before_link:
              words[i] = convert_word_to_proper_case(words, i, word_before_link, should_capitalize) + words[i][right_square_bracket_index:]
