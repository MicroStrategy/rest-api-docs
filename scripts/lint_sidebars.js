...
const generateSetFromSidebar = (fileSet, sidebarItem) => {
  if (typeof sidebarItem === "string") {
    ...
    throw Error("Sidebar item type autogenerated is not supported yet. Fix the script!!!");
  } else {
    // Force throw error, so we don't miss any unsupported cases.
    throw Error("Unknown sidebar item type. Not supported yet. Fix the script!!!");
  }
};

generateSetFromSidebar(filesInSidebar, sidebars.tutorialSidebar);

console.log("Checking missing files from sidebars.js ...");
const missingFiles = [];
files.forEach((file) => {
...
if (missingFiles.length !== 0) {
  // lint failed
  console.log("Finished checking. The following files are not in sidebars.js:");
  missingFiles.forEach((f) => console.log(f));
  exit(1);
} else {
  // Successful
  console.log("Finished checking. No missing files.");
  exit(0);
}
